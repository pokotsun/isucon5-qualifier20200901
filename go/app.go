...
func GetIndex(w http.ResponseWriter, r *http.Request) {
	...
	commentsForMe, err := FetchCommentsForMe(user.ID)
	if err != sql.ErrNoRows {
		logger.Errorw("FetchCommentsForMe", "err", err)
		checkErr(err)
	}
	...
	commentsOfFriends, err := FetchFriendComments(user.ID)
	if err != nil {
		logger.Errorw("FetchFriendComments", "err", err)
		checkErr(err)
	}

	friends := make([]Friend, 0, len(friendDict))
	for key, val := range friendDict {
